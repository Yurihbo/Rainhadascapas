...
const sellers = [
  { name: "João Carlos", initials: "JC", phone: "(11) 98824-1740", total: "R$ 350,00", status: "Pendente", updated: "Hoje, 09:42", tone: "danger" },
  ...
  return (
    <div className="page-stack page-enter">
      <section className="hero-panel" style={{ backgroundImage: `linear-gradient(90deg, rgba(15,15,15,.97) 0%, rgba(15,15,15,.82) 52%, rgba(15,15,15,.35) 100%), url(${texture})` }}>
        ...
        <div className="hero-aside"><span className="hero-aside-label">STATUS DA OPERAÇÃO</span><strong><span className="pulse-dot" /> Sincronizado</strong><small>Última atualização há 2 min</small><div className="hero-divider" /><span className="hero-aside-label">GESTÃO DO DIA</span><strong>84<span className="hero-percent">%</span></strong><small>das tarefas concluídas</small></div>
      </section>
...
function Revendedores({ onSelect }: { onSelect: (seller: typeof sellers[number]) => void }) {
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => sellers.filter((s) => `${s.name} ${s.phone}`.toLowerCase().includes(query.toLowerCase())), [query]);
  return <div className="page-stack page-enter"><div className="page-title-row"><div><span className="eyebrow">BASE DE CLIENTES</span><h1>Revendedores</h1><p>Gerencie a carteira, acompanhe valores e abra o histórico individual.</p></div><button className="gold-button" onClick={() => toast.success("Novo cadastro pronto para preenchimento") }><Plus size={16} /> Novo revendedor</button></div><div className="toolbar"><div className="search-box"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Pesquisar por nome ou telefone" /></div><button className="outline-button"><Filter size={16} /> Filtros</button><button className="outline-button">Esta semana <ChevronDown size={15} /></button></div><article className="panel table-panel"><div className="table-caption"><span>{filtered.length} revendedores encontrados</span><button className="ghost-button">Exportar lista <FileDown size={15} /></button></div><div className="table-scroll"><table><thead><tr><th>Nome</th><th>Telefone</th><th>Total da semana</th><th>Situação</th><th>Última atualização</th><th /></tr></thead><tbody>{filtered.map((seller) => <tr key={seller.name} onClick={() => onSelect(seller)}><td><div className="table-person"><div className="avatar small">{seller.initials}</div><strong>{seller.name}</strong></div></td><td>{seller.phone}</td><td className="money-cell">{seller.total}</td><td><StatusPill status={seller.status} /></td><td className="muted-cell">{seller.updated}</td><td><ChevronRight size={17} className="row-arrow" /></td></tr>)}</tbody></table></div></article></div>;
}
...
export default function Home() {
  ...
  return <div className={`app-shell ${sidebarOpen ? "sidebar-open" : "sidebar-collapsed"}`}>
    <aside className="sidebar"><div className="brand"><div className="brand-mark"><img src={logo} alt="" /></div><div><strong>RAINHA</strong><span>DAS CAPAS</span></div></div><div className="workspace-switcher"><div className="store-mark"><Store size={15} /></div><div><strong>Operação principal</strong><small>Distribuidora · SP</small></div><ChevronDown size={14} /></div><nav className="main-nav"><span className="nav-caption">MENU PRINCIPAL</span>{navItems.map(({ id, label, icon: Icon }) => <button key={id} className={active === id && !selectedSeller ? "active" : ""} onClick={() => navigate(id)}><Icon size={18} /><span>{label}</span>{id === "pagamentos" && <b className="nav-badge">5</b>}</button>)}</nav><div className="sidebar-bottom"><div className="sync-card"><div className="sync-icon"><Wifi size={15} /></div><div><strong>Sincronizado</strong><small>Dados atualizados</small></div><Check size={15} /></div><button className="profile-button"><div className="avatar tiny">YU</div><span><strong>Yuri Oliveira</strong><small>Administrador</small></span><MoreHorizontal size={16} /></button></div></aside>
    <main className="main-area"><header className="topbar"><div className="topbar-left"><button className="mobile-menu" onClick={() => setSidebarOpen((v) => !v)}><Menu size={19} /></button><button className="collapse-button" onClick={() => setSidebarOpen((v) => !v)}>{sidebarOpen ? <PanelLeftClose size={18} /> : <PanelLeftOpen size={18} />}</button><div className="breadcrumb"><span>Rainha das Capas</span><ChevronRight size={14} /><strong>{activeLabel}</strong></div></div><div className="topbar-right"><div className="offline-state"><span className="online-dot" /> Online</div><button className="topbar-icon"><Search size={18} /></button><button className="topbar-icon notification"><Bell size={18} /><i /></button><div className="top-avatar">YU</div></div></header><div className="content-area">{selectedSeller ? <SellerDetail seller={selectedSeller} onBack={() => setSelectedSeller(null)} /> : active === "dashboard" ? <Dashboard onNavigate={navigate} /> : active === "revendedores" ? <Revendedores onSelect={setSelectedSeller} /> : active === "pagamentos" ? <Payments /> : <GenericModule id={active} />}</div><footer className="app-footer"><span>Rainha das Capas · Gestão Comercial</span><span><CloudOff size={13} /> PWA pronto para uso offline</span></footer></main>
  </div>;
}
