...
function currentPeriod() { const now = new Date(); const month = String(now.getMonth() + 1).padStart(2, "0"); const week = String(Math.ceil(now.getDate() / 7)).padStart(2, "0"); const date = now.toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long", year: "numeric" }); const time = now.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }); return { label: `MÊS ${month} · SEMANA ${week}`, dateTime: `${date} · ${time}` }; }
...
function ReportsModule() {
  const [open, setOpen] = useState(false); const [report, setReport] = useState<{ title: string; type: string; week: string } | null>(null); const [title, setTitle] = useState("Relatório semanal"); const [type, setType] = useState("Detalhado"); const [week, setWeek] = useState("Semana 33 · 11–15 ago 2026");
...
export default function Home() {
  ...
  const deleteSeller = (seller: Seller) => { setSellerList((current) => current.filter((item) => item.name !== seller.name)); setSelectedSeller((current) => current?.name === seller.name ? null : current); toast.success("Revendedor removido"); };
  const updateStatus = (seller: Seller, status: string) => { const next = { ...seller, status, tone: status === "Pago" ? "success" : status === "Acordo" ? "warning" : "danger", updated: "Agora" }; setSellerList((current) => current.map((item) => item.name === seller.name ? next : item)); if (selectedSeller?.name === seller.name) setSelectedSeller(next); };
  const deleteItem = (item: SellerItem) => { if (!selectedSeller) return; const items = (selectedSeller.items ?? []).filter((row) => !(row.item === item.item && row.date === item.date)); const next = { ...selectedSeller, items, total: currency(items.reduce((sum, row) => sum + Number(row.total.replace(/[^0-9,-]/g, "").replace(".", "").replace(",", ".")), 0)), updated: "Agora" }; setSellerList((current) => current.map((seller) => seller.name === next.name ? next : seller)); setSelectedSeller(next); toast.success("Item removido"); };
